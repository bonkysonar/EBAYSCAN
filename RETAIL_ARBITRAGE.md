...
After capturing retailer pages to recover a failed or blocked scan, create a **new bounded browser recovery** with this command, replacing the previous artifact path with the exact draft or final to retain campaign context from:

```powershell
node scripts/runRetailWorkflow.mjs --browserOnly --browserObservations=exports/arbitrage-finds/browser-source-observations.json --previousScan=<exact-prior-draft-or-final.json>
```

This always starts in `refresh` mode, scans only source IDs with browser observations no older than six hours, and returns a new context, draft, checkpoint, and plan. It leaves broad-scan cadence and the previous artifact unchanged. The reported source count comes from the new scanner output. Complete sold research against the **new** plan, then run `node scripts/runRetailWorkflow.mjs --finish=<new-context-path> --browserResearch=exports/arbitrage-finds/browser-product-research.json`. Publication remains `source_updates` with bounded coverage even if every observed page succeeds; pending research stays pending. Never append retailer observations through `--finish` or reuse the previous run ID.
