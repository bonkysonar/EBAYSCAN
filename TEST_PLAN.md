...
2. Run a default sale-radar scan. Verify candidates are ranked globally before the cap, more than one source can survive the queue limit, obvious navigation/merch/non-vinyl rows are absent, and sale alerts remain separate from record-level opportunities. Include high-noise fixtures for Walmart, Barnes & Noble, and Cheap Vinyl: ISBN books, category/filter links, record players, and `Vinyl` apparel must be rejected, while an explicitly identified vinyl LP remains eligible. Verify `/ea`, `/lb`, shipping, savings, and coupon amounts cannot become the purchase price; retailer taxonomy and escaped HTML entities must not pollute the artist/title or eBay query. For Slickdeals, verify the role-labelled card parser keeps the real current/list prices, advertised discount, retailer, and thread together, preserves fractional cents, and never treats `free shipping on $35+` as a `$35` list price. For Walmart, verify the structured `$10`/`$15`/`$20` lanes retain first-party records at or below `$20` even without a markdown, paginate, deduplicate, and recover stale search-result inventory through bounded product-page availability checks.

3. On a Shopify source, verify the scan follows multiple JSON pages when available, ignores unavailable variants, and retains compare-at price, currency, SKU, barcode, variant identity, inventory, and collection context. On a mixed CD/LP product, only the LP variant may become a vinyl candidate and it must retain the LP's price and exact variant URL. Include `2LP`, `2xLP`, and `2-LP` variant spellings under contradictory CD/Vinyl product taxonomy. Reproduce a descriptive stale handle whose URL names a different record and verify it is excluded, while a normal artist/title handle and an opaque catalog-number handle remain eligible. Verify a source with `resalePolicy: prohibited` is absent from active acquisition targets.

4. With eBay credentials configured, verify active enrichment paginates, inspects all returned pages within its cap, and reports exact matched supply separately from raw listings inspected. A broad eBay result total must not be labeled exact supply. Repeat without `EBAY_DELIVERY_POSTAL_CODE`: exact matched identities/counts must still work, while shipping/landed price is marked destination-unverified and cannot hard-reject the offer or authorize `BUY`. If every child query fails, verify the parent status is `failed`, not `enriched`.
...
10. Open `#/retail-arbitrage`. Verify the default queue shows Tier A/B/C candidates ordered A > B > C even when automatic `BUY` is zero; Tier A must always be a canonical `BUY`, aggregate-only research must cap at B, and C must say it is a research lead rather than a value claim. Verify tier filters, candidate score/reasons, the separate evidence decision, source price/currency, retailer link, Product Research link, and public Sold/Completed link. Verify the detail panel still shows all three adaptive profiles, evidence score breakdown, gates, 30/90/365-day and long-window evidence, sell-through, supply horizon, and freshness/confidence.
...
- Candidate tier is the primary queue/rank: A is verified BUY, B is promising with explicit proof gaps, and C is a source-linked research lead; artist history cannot alter tier or ordering.
...
- Run `node scripts/runRetailWorkflow.mjs --browserOnly --browserObservations=<captures> --previousScan=<exact-draft-or-final>` after capturing recovery pages. Verify refresh mode even when a broad scan is due, only fresh observed source IDs are requested, the exact previous path reaches the scanner, broad cadence remains unchanged, and the new output supplies its own run/context/checkpoint/plan and actual source count. Reject stale-only captures, a context passed as the previous scan, a prior run returned as the new output, and scan arguments supplied to `--finish`. Published recovery coverage stays partial and all-pending research stays incomplete.
- Verify named Essentials/Classic series, Ghostly Blue/Apple Red colors, translucent variants and 2LP formats cannot borrow generic or incompatible pressing sales. An album search containing multiple named series supports album demand, but an unidentified pressing must not inherit exact prices. Leading VINYL/LP merchandising should not discard an otherwise exact artist-and-album purchase match.
