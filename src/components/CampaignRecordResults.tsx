...
export function CampaignRecordResults({
  ...
  const campaigns = payload?.saleEvents ?? [];
  ...
  return (
    <section className="panel">
      ...
      {campaigns.map((campaign) => {
        ...
        const rows = products.filter((f) => f.appliedSaleCampaignId === id);
          ...
          <details key={id} className="campaign-record-results">
            <summary>
              ...
              {campaign.saleDiscountPercent
                ? `${campaign.saleDiscountPercent}% offer`
                : "Basket offer"}{" "}
              · {best.length} worth considering
            </summary>
            <p>
              {counts?.retained ?? rows.length} eligible offers retained for
              research · {counts?.priced ?? 0} price verified ·{" "}
              {counts?.evidenceCompleted ?? 0} evidence complete ·{" "}
              {counts?.economicallyQualified ?? 0} meet the profit threshold ·{" "}
              {counts?.unresolved ?? 0} have unresolved terms or exclusions.
            </p>
            <p>
              {String(
            ...
            {best.length ? (
              ...
              <p>
                No records from this campaign currently clear the decision-list
                requirements.
              </p>
            )}
            <a
            ...
            {campaign.basketScenario ? (
              <p>
                Illustrative basket: {campaign.basketScenario.quantity} records,{" "}
                {campaign.basketScenario.currency}{" "}
