import { retailOperations } from "./src/server/retailOperationsApi";
...
import {
  readSoldHistoryIndex,
  searchSoldHistory,
} from "./src/server/soldHistoryApi";
...
function ebayLocalApiPlugin(): Plugin {
  return {
    ...
    configureServer(server) {
      Object.assign(process.env, readLocalEnv(process.cwd()));
      server.middlewares.use("/api/arbitrage/operations", async (req, res) => {
        try {
          const action =
          ...
          const body =
            req.method === "POST"
              ? JSON.parse(await readBody(req, 16384))
              : null;
          sendJson(
            res,
            200,
        ...
        try {
          ...
          const message =
            error instanceof Error
              ? error.message
              : "Unknown marketplace API error";
          sendJson(res, isRateLimitError(message) ? 429 : 500, {
            error: message,
          });
        }
      });
      ...
      server.middlewares.use("/api/vinyl-lots/scan", async (req, res) => {
        ...
        try {
          ...
          sendJson(
            res,
            200,
            await scanVinylLots(readLocalEnv(process.cwd()), { scanRequest }),
          );
        } catch (error) {
          const statusCode =
      ...
      server.middlewares.use("/api/vinyl-lots/feedback", async (req, res) => {
        ...
        if (!isLoopbackRequest(req)) {
          sendJson(res, 403, {
            error: "Vinyl-lot feedback can only be saved from this computer.",
          });
          return;
        }
        ...
        try {
          ...
          sendJson(
            res,
            200,
            await saveVinylLotFeedback(payload, {
          ...
          sendJson(
              ...
              : 400,
            {
              error:
                error instanceof Error
                  ? error.message
                  : "Unknown vinyl-lot feedback error",
            },
          );
        }
      });
        ...
        try {
          ...
          sendJson(res, 502, {
            error:
              error instanceof Error
                ? error.message
                : "Unknown Discogs stats pull error",
          });
        }
      });
      ...
      server.middlewares.use("/api/ebay/seller-listings", async (req, res) => {
        ...
        try {
          const result = await fetchSellerActiveListings(
            readLocalEnv(process.cwd()),
            parseSellerListingsOptions(await readBody(req)),
          );
          sendJson(res, 200, result);
        } catch (error) {
          sendJson(res, 500, {
            error:
              error instanceof Error
                ? error.message
                : "Unknown seller listings API error",
          });
        }
      });
        ...
        try {
          ...
          sendJson(res, 500, {
            error:
              error instanceof Error
                ? error.message
                : "Unknown arbitrage finds API error",
          });
        }
      });
        ...
        try {
          ...
          sendJson(res, 500, {
            error:
              error instanceof Error
                ? error.message
                : "Unknown arbitrage history API error",
          });
        }
      });
        ...
        try {
          ...
          sendJson(res, Number.isFinite(statusCode) ? statusCode : 500, {
            error:
              error instanceof Error
                ? error.message
                : "Unknown arbitrage upload API error",
          });
        }
      });
      ...
      server.middlewares.use("/api/sold-history/search", async (req, res) => {
        ...
        try {
          ...
          sendJson(res, 200, {
            results: searchSoldHistory(process.cwd(), query),
            status: "available",
          });
        } catch (error) {
          sendJson(res, 500, {
            error:
              error instanceof Error
                ? error.message
                : "Unknown sold history API error",
          });
        }
      });
    },
...
function parseSellerListingsOptions(body: string): {
  maxPages?: number;
  pageNumber?: number;
} {
  try {
